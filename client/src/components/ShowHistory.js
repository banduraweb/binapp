        ...
        <div>
            ...
            {visibleHistory.length > 0 && visibleHistory.map(({title, calories, ingredients, preparation})=>
                <Message  key={title} style={
                    ...
                    <Message.List>
                    <Message.Item><Icon  name='food' />Ingredients: &emsp;{ingredients}</Message.Item>
                    <Message.Item><Icon  name='american sign language interpreting' />Preparation: &emsp; {preparation}</Message.Item>
                    </Message.List>
                </Message>
            )}
            ...
            <Button style={{backgroundColor:"#b0d3bf"}}
                onClick={()=>{setVisibleHistory([])}}
                floated='right'>Close previous version
